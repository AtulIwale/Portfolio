# Implementation — Procurement & Subcontracting
...
Approved suppliers, items, trades, open POs, work orders, accepted receipts and prior certificates. Retain source-to-target ID mapping, reconcile totals, log rejected rows and obtain the process owner's sign-off.

## Risk and acceptance

Poor supplier/item masters or open-order migration can duplicate commitments. Use a trial migration and the module's [acceptance tests](business-analysis.md). Procurement manager owns process acceptance; the data engineer owns migration; the controller owns financial reconciliation; the sponsor authorizes go/no-go.

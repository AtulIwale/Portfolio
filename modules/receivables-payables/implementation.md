# Implementation — Accounts Receivable & Accounts Payable
...
Customers/suppliers, invoice balances, allocation history and control-account mappings. Retain source-to-target ID mapping, reconcile totals, log rejected rows and obtain the process owner's sign-off.

## Risk and acceptance

Migrating gross and open balances together can duplicate liabilities. Use a trial migration and the module's [acceptance tests](business-analysis.md). Finance controller owns process acceptance; the data engineer owns migration; the controller owns financial reconciliation; the sponsor authorizes go/no-go.

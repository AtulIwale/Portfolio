# Implementation — Construction Assets — Fixed & Movable
...
Asset register, opening book values, custodians, meter readings and service dates. Retain source-to-target ID mapping, reconcile totals, log rejected rows and obtain the process owner's sign-off.

## Risk and acceptance

Opening book values and missing maintenance histories undermine reconciliation. Use a trial migration and the module's [acceptance tests](business-analysis.md). Plant and asset manager owns process acceptance; the data engineer owns migration; the controller owns financial reconciliation; the sponsor authorizes go/no-go.

# Implementation — Construction Payroll
...
Salary structures, approved attendance, open deductions and opening balances. Retain source-to-target ID mapping, reconcile totals, log rejected rows and obtain the process owner's sign-off.

## Risk and acceptance

Cut-off changes after sign-off can invalidate the payroll parallel run. Use a trial migration and the module's [acceptance tests](business-analysis.md). Payroll lead owns process acceptance; the data engineer owns migration; the controller owns financial reconciliation; the sponsor authorizes go/no-go.

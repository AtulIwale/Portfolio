# Implementation — Construction HR
...
Fictional employee master, payroll groups, calendars and approved attendance. Retain source-to-target ID mapping, reconcile totals, log rejected rows and obtain the process owner's sign-off.

## Risk and acceptance

Duplicate employees and unapproved attendance corrupt payroll inputs. Use a trial migration and the module's [acceptance tests](business-analysis.md). HR operations lead owns process acceptance; the data engineer owns migration; the controller owns financial reconciliation; the sponsor authorizes go/no-go.
